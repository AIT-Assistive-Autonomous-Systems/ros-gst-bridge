...
static GstStaticPadTemplate rosrawsink_sink_template =
...
static gboolean rosrawsink_open(RosBaseSink * ros_base_sink)
{
  ...
  sink->pub = rclcpp::create_publisher<std_msgs::msg::ByteMultiArray>(
...
static GstFlowReturn rosrawsink_render(RosBaseSink * ros_base_sink, GstBuffer * buf, rclcpp::Time)
{
  Rosrawsink * sink = GST_ROSRAWSINK(ros_base_sink);
  ...
  // This still copies, but you could replace this with a custom message to hold the memory view
